...
namespace fluid
{
	...
	Geometry::Geometry(int Nx, int Ny, int Nz, double Lx, double Ly, double Lz) : Nx(Nx),
																				  Ny(Ny),
																				  Nz(Nz),
																				  Lx(Lx),
																				  Ly(Ly),
	...
	Geometry::Geometry(const Geometry &geo) : Geometry(geo.Nx, geo.Ny, geo.Nz, geo.Lx, geo.Ly, geo.Lz)
	{
		DeepCopy(geo);
	}
	...
	Geometry &Geometry::operator=(const Geometry &geo)
	{
		...
		{
			string errmsg = "Geometry sizes do not match !";
			cout << errmsg << endl;
	...
	void Geometry::InitMeshEdge(bool loadX, bool loadY, bool loadZ,
								const char *pathx, const char *pathy, const char *pathz)
	{
	...
	// load x mesh edge position from file
	void Geometry::InitMeshX(const char *path)
	/* read grid from file at specified path */
	{
		...
		for (int j = 0; j <= Nx; j++)
			sscanf(fgets(str, 1024, fp), "%le", &x[j]);

		fclose(fp);

		// check
		if (fabs((x[Nx] - x[1]) - Lx) > INFTSM)
		{
			string errmsg = "No valid X mesh provided !";
			cout << errmsg << endl;
			throw runtime_error(errmsg);
		}
	}
	...
	/* read grid from file at specified path */
	{
		...
		{
			string errmsg = "No valid Y mesh provided !";
			cout << errmsg << endl;
			throw runtime_error(errmsg);
		}
	}

	// load z mesh edge position from file
	void Geometry::InitMeshZ(const char *path)
	/* read grid from file at specified path */
	{
		FILE *fp;
		char str[1024];
		...
		for (int j = 0; j <= Nz; j++)
			sscanf(fgets(str, 1024, fp), "%le", &z[j]);

		fclose(fp);

		// check
		if (fabs((z[Nz] - z[1]) - Lz) > INFTSM)
		{
			string errmsg = "No valid Z mesh provided !";
			cout << errmsg << endl;
			throw runtime_error(errmsg);
		}
	}
	...
	{
		return hyptan(2, gma, N, L) - hyptan(1, gma, N, L);
	}
